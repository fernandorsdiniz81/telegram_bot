...
class TelegramBot:
	...
	def read_messages(self) -> bool:
		try: 
			read_message_url = f'https://api.telegram.org/bot{self.token}/getUpdates?timeout={self.timeout}&offset={self.update_id}' #offset é para fazer a request pelo "update_id" ao invés de baixar todo o histórico
	...
	def answer_messages(self) -> None:
		answer =  self.bot_intelligence.answer_messages_with_ai(self, self.chat_id, self.human_message) # -> LangChain object
